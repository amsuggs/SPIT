...
def apply_units_and_exponentiation():
    ...
    if vals.peak is not params.peak:
        vals.peak *= 1000
    vals.width = int(uep.apply_unit_and_exponents(params.width))
    if vals.width is not params.width:
        vals.width *= 1000
...
def gauss_pulse(chirp, x, mean=vals.peak, std_dev=vals.width):
    ...
    file_str = "OutputFiles/" + params.file + "(%s).bin" % (now.isoformat()) # append the datetime to the end of the file.
