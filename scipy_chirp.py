...
real = chirp(t, f0=f0, f1=f1, t1=t1, method=chirp_method)
imag = np.sin(np.arccos(real))
...
file_str = "OutputFiles/scipy_chirp_v0_true(%s).bin" % (now.isoformat())
...
now = datetime.datetime.now()
file_str = "OutputFiles/scipy_chirp_v0_false(%s).bin" % (now.isoformat())
file = open(file_str, "wb")
pickle.dump(sig, file)
file.close()